...
└── tests/
    ...
    ├── integration/              marked `api`; skipped without a classifier key
...
Key Vault (`CRR_ANTHROPIC_API_KEY`, `GOOGLE_SERVICE_ACCOUNT_B64`), 2 vCPU / 4 GiB, timeout 3600 s.
...
- `tests/integration` (marker `api`) — one real classification per manager against a golden page;
  skipped when no classifier key is set.
