...
- **First real run:** add repo secrets `CRR_ANTHROPIC_API_KEY` and `GOOGLE_SERVICE_ACCOUNT_B64`
...
| Symptom | What to do |
|---|---|
| Session says it cannot reach `api.anthropic.com` for the pipeline | The classifier key is not set. Set **`CRR_ANTHROPIC_API_KEY`** in the cloud environment and start a new session — the unprefixed `ANTHROPIC_API_KEY` is reserved by Claude Code on the web and is stripped from the container, so setting that name has no effect. `docs/SETUP-CREDENTIALS.md`. |
